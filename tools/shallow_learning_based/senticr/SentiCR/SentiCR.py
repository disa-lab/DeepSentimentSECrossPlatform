...
def expand_contractions(s, contractions_dict=contractions_dict):
    ...
    # return contractions_regex.sub(replace, s.decode('utf-8').lower())
...
def preprocess_text(id, text):
    # print("decoding %d %s " % (id, text))
    # comments = text.encode('ascii', 'ignore')
    try:
        comments = text.encode('ascii', 'ignore')
    except:
        print("====> Error in decoding decoding %d %s " % (id, text))
        comments = ("Adding dummy data").encode('ascii', 'ignore')
    # comments = text
    ...
    try:
        ...
        print("=============> error in handle negations")
...
class SentiCR:
    ...
    def create_model_from_training_data(self):
        ...
        for sentidata in self.training_data:
            comments = preprocess_text(sentidata.id, sentidata.text)
            training_comments.append(comments)
            training_ratings.append(sentidata.rating)
            training_ids.append(sentidata.id)


        values, counts = np.unique(training_ratings, return_counts=True)
        print(values, counts)
        ...
        print("len x_train %d and y_train %d" % (len(X_train), len(Y_train)))
        print(X_train.shape)
        print(Y_train.shape)
        # print(Y_train[:1000])
    ...
    def read_data_from_oracle(self):
        ...
        for cell_num in range(0, sheet.nrows):
            comments=SentimentData(cell_num, sheet.cell(cell_num, 0).value,sheet.cell(cell_num, 1).value)
    ...
    def get_sentiment_polarity_collection(self, texts):
        predictions=[]
        for text in texts:
            comment = preprocess_text(-1, text)
            feature_vector=self.vectorizer.transform([comment]).toarray()
            sentiment_class=self.model.predict(feature_vector)
...
def print_predictions(test_ids, predictions, output_file="output/output.csv"):
    preds = [p[0]for p in predictions]
    values, counts = np.unique(preds, return_counts=True) 
    print("preds statisctics %s %s" % (values, counts))

    df = pd.DataFrame()
    df['id'] = test_ids
    df['pred_label'] = preds
    # for i in range(len(test_ids)):
    #     print("%d %d" % (test_ids[i], predictions[i][0]))
...
def ten_fold_cross_validation(dataset, ALGO):
    ...
    for train, test in kf.split(dataset):
        ...
        test_ids = [comments.id for comments in dataset[test]]

        values, counts = np.unique(test_ratings, return_counts=True)
        print(values, counts)


        pred = classifier_model.get_sentiment_polarity_collection(test_comments)
        # print(pred)
        print_predictions(test_ids, pred)
...
def cross_platform_validation(training_dataset, test_dataset, ALGO, output_file="output/output.csv"):

    classifier_model = SentiCR(algo=ALGO, training_data = training_dataset)

    print("Model has been created")

    test_comments = [comments.text for comments in test_dataset]
    test_ratings = [comments.rating for comments in test_dataset]
    test_ids = [comments.id for comments in test_dataset]
    ...
    pred = classifier_model.get_sentiment_polarity_collection(test_comments)
    # print(pred)
    print_predictions(test_ids, pred, output_file=output_file)

    precision = precision_score(test_ratings, pred, average="micro")
    recall = recall_score(test_ratings, pred,  average="micro")
    f1score = f1_score(test_ratings, pred,  average="micro")
    accuracy = accuracy_score(test_ratings, pred)
...
if __name__ == '__main__':
    ...
    cross_platform_data_file = "/home/mdabdullahal.alamin/alamin/sentiment/cross_platform/dataset/processed/combined.csv"
    ...
    if(type == "cross" or type =="both"):
        for train_dataset in datasets:
            ...
            train_df, test_df = disa_create_cross_platform_train_test_set(cross_platform_data_file, train_dataset)
            print("train_df len: %d and test_df len %d " % (len(train_df), len(test_df)))
            ...
            output_file = "output/senticr_train_" + train_dataset + ".csv"
            precision, recall, f1score, accuracy = cross_platform_validation(train_sentences, test_sentences, ALGO, output_file=output_file)
        ...
        for dataset in datasets:
            ...
            for fold in range(10):
                ...
                for index, row in test_df.iterrows():
                    ...
                    test_sentences.append(sentence)
                train_sentences = train_sentences
                output_file = "output/senticr_" + dataset + "_" + str(fold) + ".csv"
                precision, recall, f1score, accuracy = cross_platform_validation(train_sentences, test_sentences, ALGO, output_file=output_file)
                print("Precision:"+str(precision))
                print("Recall:" + str(recall))
                print("F-measure:" + str(f1score))
    ...
    # ===========================================
            ...
            # train_file = "/home/mdabdullahal.alamin/alamin/sentiment/senticr/SentiCR/input/datasetsenti4sdso.csv"
            # train_file = "/home/mdabdullahal.alamin/alamin/sentiment/senticr/SentiCR/input/ortujira.csv"

            # test_file = "/home/mdabdullahal.alamin/alamin/sentiment/senticr/SentiCR/input/ortujira.csv"
            # test_file = "/home/mdabdullahal.alamin/alamin/sentiment/senticr/SentiCR/input/github.csv"

            # train_df = pd.read_csv(train_file, skiprows=1, names=['id', 'oracle', 'text'], dtype={'oracle': 'float'})
