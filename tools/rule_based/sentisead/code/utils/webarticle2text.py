...
This module is heavily copied from the github repo with the same name:
https://github.com/chrisspen/webarticle2text/blob/master/webarticle2text.py
@author: gias
'''
"""
...
# copied from: http://www.diveintopython.net/http_web_services/redirects.html
...
class HTMLParserNoFootNote(HTMLParser.HTMLParser):
    """
...
def fetch(url, timeout=5, userAgent=None, only_mime_types=None):
    try:
        valid, the_url =  nlputils.validUrl(url) 
        if valid == False: return None, None
        else:
            print "the_url = %s" %(the_url)
            r = requests.get(the_url, timeout = timeout)
            response = r
        #return r.text
    except requests.exceptions.HTTPError:
        print "could not download the page in %s" %(url)
        return None, None
    except requests.exceptions.MissingSchema:
        print "Invalid schema for url %s. Discarding." %(url)
        return None, None
    except requests.exceptions.ConnectionError:
        print "Connection error for url %s"  %(url)
        return None, None
    except requests.exceptions.Timeout:
        print "Connection timeout for url %s"  %(url)
        return None, None
    except requests.exceptions.ContentDecodingError:
        print "Content decoding error for url %s" %(url)
        return None, None
    except requests.exceptions.InvalidURL:
        print "invalid url %s" %(url)
        return None, None
    except requests.exceptions.TooManyRedirects:
        print "too many redircts for url %s" %(url)
        return None, None
    ...
    if only_mime_types:
        ...
        if not ct:
            ...
            ct = (r.headers('content-type') or '').split(';')[0]
...
def check_robotstxt(url, useCache, cache_dir, userAgent=None):
    ...
    robotstxt_url = urllib2.urlparse.urlunsplit((
        scheme,
        netloc,
        '/robots.txt',
        '',
    ...
    for k, v in urllib2.OpenerDirector().addheaders:
        if k == "User-agent":
            default_useragent = v
            break
...
def extractFromURL(url,
    ...
    html, encoding = fetch(url,timeout=timeout,userAgent=userAgent,only_mime_types=only_mime_types)
