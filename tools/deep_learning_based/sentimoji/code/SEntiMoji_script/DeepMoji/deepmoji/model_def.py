...
def load_specific_weights(model, weight_path, nb_tokens, maxlen, exclude_names=[], nb_classes=64):
    ...
    for layer_pretrain in model_pretrain.layers:
        if len(layer_pretrain.trainable_weights) == 0:
            continue
        if layer_pretrain.name in exclude_names:
            continue
        layer_model = model.get_layer(name=layer_pretrain.name)
        layer_model.set_weights(layer_pretrain.get_weights())
    del model_pretrain
    gc.collect()
    return model
