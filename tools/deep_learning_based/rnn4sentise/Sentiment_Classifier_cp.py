...
print("++++++++++++++==================== >>  Going to disable TF 2.x behaviour")
import tensorflow.compat.v1 as tf
tf.disable_v2_behavior()
print("Going to disable eager execution to ensure compability with TF 2.x")
...
print("Goint to open file: %s" % (file_path))
with open(file_path, "r") as csvfile:
    reader=csv.DictReader(csvfile)
    for row in reader:
        indexCounter = 0
...
print("Sentence counter %d" % (sentence_counter))
print("Going to save idsMatrix for reuse")
# Save the ids array for reuse
np.save('idsMatrix_%s_%s' % (m, d), ids)

# Load the ids array
print("Going to load idsMatrix from saved file")
...
labels = tf.compat.v1.placeholder(tf.float32, [batch_dim, numClasses])
input_data = tf.compat.v1.placeholder(tf.int32, [batch_dim, maxSeqLength])



# Put the vocab size and the vector dimension in here
Wembed = tf.compat.v1.placeholder(tf.float32, [Vocab_Size, numDimensions], name='Wembed')
...
def divide_batch():
    print("Divide_bath method called")
    ...
    print("Going to write to a csv file")
...
def divide_sub_batches(temp_batch, index_start, index_end):
    print("divide_sub_batches called")
...
def write_log(s):
    ...
    print("Going to write to actualpredictions_ file")
...
print("size of train_batch: %d size of test_batch %d" % (len(train_batches), len(test_batches)))
    ...
    for epochx in range(50):
        for x in range(len(train_batches)):
            if x != fold:
                ...
                print("Commented out last loop not required")
